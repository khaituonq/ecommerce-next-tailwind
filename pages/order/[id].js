...
const OrderScreen = () => {
    ...
    const { shippingAddress, orderItems, itemsPrice, shippingPrice, totalPrice, isDelivered, deliveredAt } = order;

    // function createOrder(date, actions) {
    ...
    return (
        <Layout title="Order Status">
            <h1 className="mb-4 text-xl uppercase font-medium">{`Order Status - ${orderId.substring(17)}`}</h1>
            {loading ? (
                <div>Loading...</div>
            ) : error ? (
